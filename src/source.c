	...
	if (slowpath(ke->flags & EV_ERROR)) {
		if (ke->filter == EVFILT_PROC && ke->data == ESRCH) {
			...
			if (ke->flags & EV_DELETE) {
				_dispatch_debug("kevent[0x%"PRIx64"]: ignoring ESRCH from "
						"EVFILT_PROC EV_DELETE", ke->udata);
				return;
			}
			_dispatch_debug("kevent[0x%"PRIx64"]: ESRCH from EVFILT_PROC: "
...
{
	dispatch_source_refs_t dr = ds->ds_refs;
	return dsnprintf(buf, bufsiz, "timer = { target = 0x%"PRIx64", "
			"deadline = 0x%"PRIx64", last_fire = 0x%"PRIx64", "
			"interval = 0x%"PRIx64", flags = 0x%lx }, ",
